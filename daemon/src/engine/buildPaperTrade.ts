...
export function buildTrade(
  ...
  // Cost-aware R:R gate: require net R:R *after* NSE intraday charges (STT/brokerage/GST/…) to
  // still clear a floor — rejects setups where costs eat the whole edge. 0.4 (was 1.65): that
  // threshold was calibrated for the old far T2 target (~2.5R gross); T1-only targets (removed
  // the ladder 2026-08-27) run ~0.7-0.83R gross by construction, so 1.65 silently rejected every
  // single trade — same bug class as the two 1.5 gates fixed the same day, just missed then.
  // 0.4 sits below the 0.5 gross-R:R floor used elsewhere (canTradeRow/planFromLevelsT1T2's
  // MIN_RR) since this version additionally nets out real transaction costs.
  ...
  if (!Number.isFinite(netRR) || netRR < 0.4) return null;
