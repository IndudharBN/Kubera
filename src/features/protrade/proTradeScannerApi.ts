...
export async function fetchHotSetSnapshot(symbols: string[]): Promise<ProTradeRow[]> {
  ...
  const nifty50Trend5m = candleTrend(nifty505mBars['SPY'] || []);
  const nifty50Trend15m = candleTrend(nifty5015mBars['SPY'] || []);
  ...
  const nifty50H1 = (nifty50H1Bars['SPY'] || []).slice(-5);
  ...
  return symbols.flatMap((sym) => {
    ...
    return [buildRowFromAlpaca(sym, meta, candleSet, providerStatus, newsFlags[sym] ?? 'none', sectorTrends, earningsDays, nifty50ChangePct, vixLevel, nifty50Trend5m, nifty50Trend15m, nifty50RegimeData.spyBars)];
  });
}
...
export async function fetchProTradeScannerSnapshot(pinnedSymbols: string[] = []): Promise<ProTradeSnapshot> {
  ...
  const nifty50H1 = (nifty50Bars['SPY'] || []).slice(-5);
  const nifty50Last = nifty50H1.length >= 2 ? nifty50H1[nifty50H1.length - 1].close : 0;
  const nifty50Base = nifty50H1.length >= 4 ? nifty50H1[nifty50H1.length - 4].close : (nifty50H1.length >= 2 ? nifty50H1[nifty50H1.length - 2].close : nifty50Last);
  const nifty50ChangePct = nifty50Base > 0 ? (nifty50Last - nifty50Base) / nifty50Base : 0;

  const nifty505m = (nifty505mBars['SPY'] || []);
  const nifty50Trend5m = candleTrend(nifty505m);
  const nifty50Trend15m = candleTrend(nifty5015mBars['SPY'] || []);

  // Macro regime: NIFTY50 EMA200 (daily) + VIX
  const nifty50DailyCloses = nifty50RegimeData.spyBars.map((c) => c.close);
  ...
  const nifty50DailyPrice = nifty50RegimeData.spyBars.length ? last(nifty50RegimeData.spyBars).close : null;
  const vixLevel = nifty50RegimeData.vixLevel;
